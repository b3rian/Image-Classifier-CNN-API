...
def main():
    ...
    # Model selection (disabled when comparing)
        ...
        for idx, (img, name) in enumerate(images):
            ...
            with col2:
                ...
                if st.button("🚀 Classify Image", key=f"btn_{idx}", type='primary'):
                    if compare_models:
                        # Run both models
                        result1 = classify_image(img, "efficientnet")
                        result2 = classify_image(img, "resnet")
                        ...
                        if result1 and result2:
                            ...
                            with col2:
                                ...
                                display_predictions(
                                    ...
                                    result2['inference_time']
                                )

                            # Log both results in history
                            timestamp = datetime.now().isoformat()
                            st.session_state.history.append({
                                "name": name + " (EfficientNet)",
                                "predictions": result1['predictions'],
                                "model": result1['model_version'],
                                "time": timestamp
                            })
                            st.session_state.history.append({
                                "name": name + " (ResNet)",
                                "predictions": result2['predictions'],
                                "model": result2['model_version'],
                                "time": timestamp
                            })
                    else:
